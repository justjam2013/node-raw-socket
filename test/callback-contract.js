...
const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'raw-callback-contract-'));
...
function verify(run) {
  ...
  assert.match(run.stdout, /^REQUIRED callback contract PASS$/m);
}
try {
  ...
  const probes = [
    ['void SocketWrap::HandleIOEvent (int status, int revents) {',
      'if (!status) { if (revents & UV_READABLE) ++readable; if (revents & UV_WRITABLE) ++writable; }'],
    ['NAN_METHOD(SocketWrap::Recv) {', '++receives;'],
    ['NAN_METHOD(SocketWrap::Send) {', '++sends;']
  ];
  probes.forEach(([needle, probe]) => {
    source = replaceOnce(source, needle, needle + '\n' + probe);
  });
  ...
  fs.copyFileSync(path.join(__dirname, 'fixtures/callback-contract.cc'), path.join(directory, 'harness.cc'));
  const addon = require('./helpers/build-addon')(directory);
  const args = [path.join(__dirname, 'fixtures/callback-contract.js'), addon];
